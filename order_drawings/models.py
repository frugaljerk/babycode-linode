...
class MyBabyDrawings(models.Model):
    ...
    def upload_path(self, filename):
        ...
        self.counter += 1
        return f"order_drawings/order_{str(self.order_id)}/{filename}"
    ...
    confirm_status = models.BooleanField(
        default=False
    )  # True when user confirm drawing of uploaded pics
    status = models.BooleanField(
        default=False
    )  # True for Order complete and ready for download, False for Order in progress
...
@receiver(post_delete, sender=MyBabyDrawings)
def mymodel_delete(sender, instance, **kwargs):

    path_to_folder = os.path.join(settings.MEDIA_ROOT, 'order_drawings', rf'order_{instance.order_id}').replace("\\", '/')
    try:
